...
//                    bool isBackwards = newWaypointIndex < prevWPIndex;
                    ...
                    if (isBackwards) {
//                        for (int i = prevWPIndex - 1; i > newWaypointIndex - 1; --i) Tween.OnTweenCallback(t.onWaypointChange, i, t.target);
                        for (int i = prevWPIndex - 1; i > newWaypointIndex - 1; --i) Tween.OnTweenCallback(t.onWaypointChange, i, t.target, t.intId);
                    } else {
//                        for (int i = prevWPIndex + 1; i < newWaypointIndex + 1; ++i) Tween.OnTweenCallback(t.onWaypointChange, i, t.target);
                        for (int i = prevWPIndex + 1; i < newWaypointIndex; ++i) Tween.OnTweenCallback(t.onWaypointChange, i, t.target, t.intId);
                    }
                    Tween.OnTweenCallback(t.onWaypointChange, newWaypointIndex, t.target, t.intId);
                }
            }
        }
