...
namespace DG.Tweening
{
    ...
    {
        internal const int idUnset = -999;
        internal TweenStreamingData streamingData;
        internal static event Action<Tween> TrackReset;
        ...
        public int intId = idUnset;
        /// <summary>long ID (usable for filtering with DOTween static methods).
        /// Default is -999 so avoid using an ID like that or it will capture all unset longIds</summary>
        public long longId = idUnset;
...
//            updateType = UpdateType.Normal;
        ...
        internal static bool DoGoto(Tween t, float toPosition, int toCompletedLoops, UpdateMode updateMode)
        {
            ...
            if (!t.playedOnce && updateMode == UpdateMode.Update) {
                t.playedOnce = true;
                if (t.onStart != null) {
                    OnTweenCallback(t.onStart, t.target, t.intId);
                    if (!t.active) return true; // Tween might have been killed by onStart callback
                }
                if (t.onPlay != null) {
                    OnTweenCallback(t.onPlay, t.target, t.intId);
        ...
        internal static bool OnTweenCallback(TweenCallback callback, object target, int id)
        {
            if (DOTween.useSafeMode) {
                try {
                    callback();
                } catch (Exception e) {
                    HandleException(target, id, e);
        ...
        internal static bool OnTweenCallback<T>(TweenCallback<T> callback, T param, object target, int id)
        {
            if (DOTween.useSafeMode) {
                try {
                    callback(param);
        ...
        static readonly MethodInfo method = typeof(UnityEngine.Object)
            .GetMethod("FindObjectFromInstanceID", BindingFlags.NonPublic | BindingFlags.Static)!;
        internal static void HandleException(object target, int id, Exception e) {
            //We use the intId to bind the tween to an object via it's instance id.
            //if the object has already been destroyed, we ignore this exception as it will likely not cause any damage.
            if(id != idUnset) {
                var obj = method.Invoke(null, new object[] { id }) as UnityEngine.Object;
                if (obj == null) return;
            }
            
            if (Debugger.logPriority >= 0) {
                Debugger.LogWarning(string.Format(
                    "An error inside a tween callback was silently taken care of. Target: '{0}' Exception\n{1}", target, e
                ));
            }
            DOTween.safeModeReport.Add(SafeModeReport.SafeModeReportType.Callback);
        }
