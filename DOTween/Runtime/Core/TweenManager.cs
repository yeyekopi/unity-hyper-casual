...
namespace DG.Tweening.Core
{
    ...
    {
        internal static event Action<Tween, bool, UpdateMode>        OnComplete;
        internal static event Action<Tween, bool>                    OnDespawn;
        internal static event Action<Tween>                          OnFlip;
        internal static event Action<Tween, float, bool, UpdateMode> OnGoto;
        internal static event Action<Tween>                          OnPause;
        internal static event Action<Tween>                          OnPlay;
        internal static event Action<Tween>                          OnPlayForward;
        internal static event Action<Tween>                          OnPlayBackwards;
        internal static event Action<Tween, bool>                    OnRewind;
        internal static event Action<Tween>                          OnSmoothRewind;
        internal static event Action<Tween, bool, float>             OnRestart;
        ...
        internal static TCore GetTweener<TCore, T1, T2, TPlugOptions>()
        ...
        internal static void SetUpdateType(Tween t, UpdateType updateType)
        {
            ...
            SetUpdateType(t, updateType, false);
        ...
        {
            OnDespawn?.Invoke(t, modifyActiveLists);
            Despawn_NoTrack(t, modifyActiveLists);
        }
        
        internal static void Despawn_NoTrack(Tween t, bool modifyActiveLists = true) {
            // Callbacks
            if (t.onKill != null) Tween.OnTweenCallback(t.onKill, t.target, t.intId);
...
//            maxActive = tweenersCapacity;
        ...
        internal static void Update(UpdateType updateType, float deltaTime, int streamFilter = -1)
        {
            if (_requiresActiveReorganization) ReorganizeActiveTweens();
...
//            Debug.Log("::::::::::: " + updateType + " > " + (_maxActiveLookupId + 1));
            ...
            for (int i = 0; i < len; ++i) {
                ...
                if (streamFilter >= 0 && (t.longId >> 32) != streamFilter) {
                    continue;
                }
...
//                if (tDeltaTime <= 0) continue; // Skip update in case time is 0 (commented in favor of next line because this prevents negative timeScales)
                ...
                if (!t.delayComplete) {
                    ...
                    if (t.playedOnce && t.onPlay != null) {
                        ...
                        Tween.OnTweenCallback(t.onPlay, t.target, t.intId);
                    }
                }
...
//                        }
         ...
         internal static Tween FindTween(string tweenId) {
            for (int i = _maxActiveLookupId; i > -1; --i)
            {
                Tween t = _activeTweens[i];
                if (t != null && t.active && t.stringId == tweenId)
                    return t;
            }
            return null;
        }
        internal static Tween FindTween(int tweenId) {
            for (int i = _maxActiveLookupId; i > -1; --i)
            {
                Tween t = _activeTweens[i];
                if (t != null && t.active && t.intId == tweenId)
                    return t;
            }
            return null;
        }
        internal static Tween FindTween(long tweenId) {
            for (long i = _maxActiveLookupId; i > -1; --i)
            {
                Tween t = _activeTweens[i];
                if (t != null && t.active && t.longId == tweenId)
                    return t;
            }
            return null;
        }
        ...
        internal static int FilteredOperation(OperationType operationType, FilterType filterType, object id, bool optionalBool, float optionalFloat, object optionalObj = null, object[] optionalArray = null)
        {
            ...
            for (int i = _maxActiveLookupId; i > -1; --i) {
                ...
                case FilterType.All:
                    ...
                    break;
                case FilterType.TargetOrId:
                    if (useStringId) isFilterCompliant = t.stringId != null && t.stringId == stringId || t.target != null && t.target.Equals(stringId);
                    else if (useLongId) isFilterCompliant = t.longId == longId || t.target != null && t.target.Equals(longId);
                    else if (useIntId) isFilterCompliant = t.intId == intId || t.target != null && t.target.Equals(intId);
                ...
                case FilterType.TargetAndId:
                    ...
                    else if (useLongId) isFilterCompliant = t.target != null && t.longId == intId && optionalObj != null && optionalObj.Equals(t.target);
                ...
                case FilterType.AllExceptTargetsOrIds:
                    ...
                    for (int c = 0; c < optionalArrayLen; ++c) {
                        ...
                        if (objId is string s) {
                            useStringId = true;
                            stringId = s;
                        } else if (objId is int id1) {
                            useIntId = true;
                            intId = id1;
                        }
                        if (useStringId && t.stringId == stringId) {
                            ...
                            break;
                        } else if (useLongId && t.longId == longId) {
                            isFilterCompliant = false;
                            break;
...
//                        if (isUpdateLoop) t.active = false; // Just mark it for killing, so the update loop will take care of it
        ...
        {
            OnComplete?.Invoke(t, modifyActiveLists, updateMode);
            return Complete_NoTrack(t, modifyActiveLists, updateMode);
        }
        
        internal static bool Complete_NoTrack(Tween t, bool modifyActiveLists = true, UpdateMode updateMode = UpdateMode.Goto)
        {
        ...
        {
            OnFlip?.Invoke(t);
            return Flip_NoTrack(t);
        }
        
        internal static bool Flip_NoTrack(Tween t)
        {
        ...
        {
            OnGoto?.Invoke(t, to , andPlay, updateMode);
            return Goto_NoTrack(t, to, andPlay, updateMode);
        }
        
        // Returns TRUE if there was an error and the tween needs to be destroyed
        internal static bool Goto_NoTrack(Tween t, float to, bool andPlay = false, UpdateMode updateMode = UpdateMode.Goto)
        {
...
//            int toCompletedLoops = (int)((decimal)to / (decimal)t.duration); // Takes care of floating points imprecision (nahh doesn't work correctly either)
            ...
            if (!andPlay && wasPlaying && !needsKilling && t.onPause != null) Tween.OnTweenCallback(t.onPause, t.target, t.intId);
            return needsKilling;
        }
        ...
        {
            OnPause?.Invoke(t);
            return Pause_NoTrack(t);
        }
        
        // Returns TRUE if the given tween was not already paused
        internal static bool Pause_NoTrack(Tween t)
        {
            if (t.isPlaying) {
                t.isPlaying = false;
                if (t.onPause != null) Tween.OnTweenCallback(t.onPause, t.target, t.intId);
                return true;
            }
            return false;
        }
        ...
        {
            OnPlay?.Invoke(t);
            return Play_NoTrack(t);
        }
        
        internal static bool Play_NoTrack(Tween t)
        {
        ...
        {
            OnPlayBackwards?.Invoke(t);
            return PlayBackwards_NoTrack(t);
        }
        
        internal static bool PlayBackwards_NoTrack(Tween t)
        {
        ...
        {
            OnPlayForward?.Invoke(t);
            return PlayForward_NoTrack(t);
        }
        
        internal static bool PlayForward_NoTrack(Tween t)
        {
            if (t.isComplete) {
                t.isBackwards = false;
                t.isPlaying = false;
        ...
        {
            OnRestart?.Invoke(t, includeDelay, changeDelayTo);
            return Restart_NoTrack(t, includeDelay, changeDelayTo);
        }
        
        internal static bool Restart_NoTrack(Tween t, bool includeDelay = true, float changeDelayTo = -1)
        {
            bool wasPaused = !t.isPlaying;
            t.isBackwards = false;
            if (changeDelayTo >= 0) t.delay = changeDelayTo;
            Rewind_NoTrack(t, includeDelay);
            t.isPlaying = true;
        ...
        {
            OnRewind?.Invoke(t, includeDelay);
            return Rewind_NoTrack(t, includeDelay);
        }
        
        internal static bool Rewind_NoTrack(Tween t, bool includeDelay = true)
        {
            ...
            if (t.position > 0 || t.completedLoops > 0 || !t.startupDone) {
                ...
                if (!needsKilling && wasPlaying && t.onPause != null) Tween.OnTweenCallback(t.onPause, t.target, t.intId);
            } else {
                // Alread rewinded
        ...
        {
            OnSmoothRewind?.Invoke(t);
            return SmoothRewind_NoTrack(t);
        }
        
        internal static bool SmoothRewind_NoTrack(Tween t)
        {
            bool rewinded = false;
            if (t.delay > 0) {
                rewinded = t.elapsedDelay < t.delay;
        ...
        internal static List<Tween> GetTweensById(int id, bool playingOnly, List<Tween> fillableList = null) {
            if (_requiresActiveReorganization) ReorganizeActiveTweens();
            int len = totActiveTweens;
            fillableList ??= new List<Tween>(len);
            for (int i = 0; i < len; ++i) {
                var t = _activeTweens[i];
                if (t != null && t.intId == id && (!playingOnly || t.isPlaying)) {
                    fillableList.Add(t);
                }
            }
            return fillableList;
        ...
        internal static List<Tween> GetTweensById(long id, bool playingOnly, List<Tween> fillableList = null) {
            if (_requiresActiveReorganization) ReorganizeActiveTweens();
            int len = totActiveTweens;
            fillableList ??= new List<Tween>(len);
            for (int i = 0; i < len; ++i) {
                var t = _activeTweens[i];
                if (t != null && t.longId == id && (!playingOnly || t.isPlaying)) {
                    fillableList.Add(t);
                }
            }
            return fillableList;
        ...
        internal static List<Tween> GetTweensById(object id, bool playingOnly, List<Tween> fillableList = null)
        {
            ...
            if (totActiveTweens <= 0) return fillableList;
            int len = totActiveTweens;
            if (fillableList == null) fillableList = new List<Tween>(len);
            // Determine ID to use
            bool useStringId = false;
